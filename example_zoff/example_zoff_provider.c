...
static int
example_copy_from_mem(zmv_t *mv, void *buf, size_t size)
{
	return kernel_offloader_copy_from_mem(mv->handle, mv->offset,
	    buf, size)?ZOFF_OK:ZOFF_ERROR;
}

static int
example_copy_to_mem(zmv_t *mv, void *buf, size_t size)
{
	return kernel_offloader_copy_to_mem(mv->handle, mv->offset,
	    buf, size)?ZOFF_OK:ZOFF_ERROR;
}
...
static int
example_all_zeros(void *handle)
{
	return (translate_rc(kernel_offloader_all_zeros(handle)));
}

static int
example_gang_add(void *gang_handle, void *new_member_handle)
{
	return translate_rc(kernel_offloader_gang_add(gang_handle,
	    new_member_handle));
}

static int
example_compress(enum zio_compress alg,
    ...
    uint64_t spa_min_alloc, zcr_t *zoff_ret)
{
	...
	const int kz_rc = kernel_offloader_compress(alg, src, dst, s_len, level,
	    spa_min_alloc, kz_ret_handle);
	if (kz_rc == KERNEL_OFFLOADER_OK) {
		/*
		 * provider doesn't have direct
		 * access to the offloader's data
		 */
		kocr_t kz_ret;
		kernel_offloader_copy_to_mem(kz_ret_handle, 0,
		    &kz_ret, sizeof (kz_ret));

		/*
		 * translate offloader extra return
		 *  values to zoff return values
		 */
...
static int
example_decompress(enum zio_compress alg,
    void *src, void *dst,
    int level)
{
	return (translate_rc(kernel_offloader_decompress(alg, src,
	    dst, level)));
}
...
static int
example_checksum_error(enum zio_checksum alg, zio_byteorder_t order,
    void *abd, void *cksum,
    ...
    void *zbc_checksum_name)
{
	...
	/* trigger offloader to do actual calculation */
	return translate_rc(kernel_offloader_checksum_error(alg, order,
	    abd, cksum,
	    encrypted, dedup,
...
int
example_write_disk(struct block_device *bdev, void *handle,
