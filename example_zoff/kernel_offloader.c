...
/* ZFS headers */
#include <zfs_fletcher.h>  /* use ZFS implementation of fletcher instead of reimplementing */
...
/* value used to swizzle the pointer so that dereferencing the handle will fail */
...
void kernel_offloader_free_local(koh_t *koh) {
	if (koh) {
		switch (koh->type) {
			case KOH_REAL:
...
static int kernel_offloader_checksum_native(koh_t *data, size_t size, uint64_t *checksum) {
	/* treat checksum as fletcher_4_ctx_t since scalar (zio_cksum_t) is the first member of the struct */
	fletcher_init((zio_cksum_t *) checksum);
	fletcher_4_scalar_native((fletcher_4_ctx_t *) checksum, ptr_start(data, 0), size);
	return KERNEL_OFFLOADER_OK;
}

/* duplicate of zio_checksum_handle_crypt */
static void kernel_offloader_checksum_handle_crypt(uint64_t *cksum, uint64_t *saved, int xor) {
	if (xor) {
		cksum[0] ^= cksum[2];
		cksum[1] ^= cksum[3];
	}
...
int kernel_offloader_checksum_compute(enum zio_checksum alg, zio_byteorder_t order,
	...
	/* compute checksum */
	if (kernel_offloader_checksum_native(data_koh, size, cksum) != KERNEL_OFFLOADER_OK) {
		return KERNEL_OFFLOADER_ERROR;
	}
...
int kernel_offloader_checksum_error(enum zio_checksum alg, zio_byteorder_t order,
	...
	if (zbc_expected_koh) {
		memcpy(ptr_start(zbc_expected_koh, 0), expected_cksum, sizeof(expected_cksum));
	}

	if (zbc_actual_koh) {
		memcpy(ptr_start(zbc_actual_koh, 0), actual_cksum, sizeof(actual_cksum));
	}
...
int kernel_offloader_decompress(enum zio_compress alg,
	...
	if ((ZIO_COMPRESS_GZIP_1 <= alg) &&
		(alg <= ZIO_COMPRESS_GZIP_9)) {
		status = kernel_offloader_gzip_decompress(src_koh, dst_koh, level, &d_len);
	}
...
int kernel_offloader_write_file(zfs_file_t *fp, void *handle, size_t count,
	...
	*err = 0;
	...
	if (koh_is_linear(koh)) {
		*err = zfs_file_pwrite(fp, ptr_start(koh, 0), count, offset, resid);
	}
		...
		/* might want to check that the amount of data written is the same as count */
	}
...
int kernel_offloader_write_disk(struct block_device *bdev, void *handle,
	...
	return kernel_offloader_vdev_disk_physio(bdev, koh, io_size, io_offset, rw, failfast, flags, zio);
}
