...
static const size_t ZFS_CHECKSUM_SIZE = sizeof(zio_cksum_t);
static const size_t ZOFF_NAME_LEN = 1024;
static const size_t BP_SIZE = ZFS_CHECKSUM_SIZE;
...
static
boolean_t zoff_provider_sane(const zoff_functions_t *provider) {
	const int required = (
		provider                 &&
		provider->alloc          &&
		provider->free           &&
...
int zoff_provider_init(const char *name, const zoff_functions_t *provider) {
	if (zoff_provider_sane(provider) != B_TRUE) {
		#ifdef _KERNEL
		printk("ZOFF Provider %s does not provide a valid set of functions\n", name);
		#endif
		return ZOFF_ERROR;
	}
...
void zoff_provider_exit(const char *name) {
	zoff_provider = NULL;
...
void zoff_on(objset_t *os, const char *name) {
	os->os_encrypted = B_FALSE;
	os->os_dedup_verify = B_FALSE;
	#ifdef _KERNEL
	printk("ZOFF %s enabled. Disabling encryption and dedup_verify.\n", name);
	#endif
}

void zoff_off(objset_t *os, const char *name) {
...
static
zhe_t *create_zhe(void *key, size_t size) {
	if (!zoff_provider) {
		return NULL;
	}
...
int zoff_alloc(void *key, size_t size) {
...
static
void zoff_free_private(void *key, boolean_t lock) {
	if (lock == B_TRUE) {
		zoff_hash_context_write_lock(&ZOFF_HANDLES);
	}
...
static
int zoff_offload(void *key, void *buf, size_t size) {
	if (!zoff_provider) {
		return ZOFF_ERROR;
	}
...
int zoff_onload(void *key, void *buf, size_t size) {
	if (!zoff_provider) {
		return ZOFF_ERROR;
	}
	...
	zhe_t *zhe = zoff_hash_find_and_remove(&ZOFF_HANDLES, key);
	if (!zhe) {
		/* either already onloaded or missing record - either way can't do anything */
		zoff_hash_context_write_unlock(&ZOFF_HANDLES);
		return ZOFF_ERROR;
	}
	...
	/* if failure, can't do anything with zhe in any case, so destroy it */
	destroy_zhe(zhe);
...
static
int zoff_onload_cb(void *buf, size_t size, void *private) {
	if (!zoff_provider) {
		return ZOFF_ERROR;
	}
...
static
int zoff_onload_abd_private(abd_t *abd, size_t size, boolean_t lock, boolean_t remove) {
	if (!zoff_provider) {
...
int zoff_onload_abd(abd_t *abd, size_t size) {
	return zoff_onload_abd_private(abd, size, B_TRUE, B_TRUE);
}

void zoff_lock_raidz(void) {
	zoff_hash_context_write_lock(&ZOFF_HANDLES);
}

void zoff_unlock_raidz(void) {
	zoff_hash_context_write_unlock(&ZOFF_HANDLES);
}

static
boolean_t zoff_has_raidz(zoff_prop_t *zoff, int raidn, boolean_t gen, boolean_t rec) {
	if (!zoff_provider               ||
	...
	if (gen == B_TRUE) {
		switch (raidn) {
			case 1:
				good = (zoff_provider->raid.gen.z1 && (zoff->raidz1_gen == 1));
				break;
			case 2:
				good = (zoff_provider->raid.gen.z2 && (zoff->raidz2_gen == 1));
				break;
			case 3:
				good = (zoff_provider->raid.gen.z3 && (zoff->raidz3_gen == 1));
				break;
			default:
				break;
		}
	...
	if (rec == B_TRUE) {
		switch (raidn) {
			case 1:
				good &= (zoff_provider->raid.rec.z1 && (zoff->raidz1_rec == 1));
				break;
			case 2:
				good &= (zoff_provider->raid.rec.z2 && (zoff->raidz2_rec == 1));
				break;
			case 3:
				good &= (zoff_provider->raid.rec.z3 && (zoff->raidz3_rec == 1));
				break;
			default:
				break;
		}
...
int zoff_alloc_raidz(zio_t *zio, raidz_row_t *rr) {
	if (!zio || !rr) {
		return ZOFF_ERROR;
	}
	...
	/* allocates the rr offloader struct, but does not fill in the column data */
	void *rr_handle = zoff_provider->raid.alloc(rr->rr_firstdatacol, rr->rr_cols);
	if (!rr_handle) {
		return ZOFF_ERROR;
	}
	...
	for(uint64_t c = 0; (c < rr->rr_firstdatacol) && (good == B_TRUE); c++) {
		zhe_t *zhe = create_zhe(rr->rr_col[c].rc_abd, rr->rr_col[c].rc_size);
		if (!zhe) {
			good = B_FALSE;
			break;
		}
...
static
int zoff_free_raidz_private(raidz_row_t *rr, boolean_t lock) {
	if (!rr) {
		return ZOFF_ERROR;
	}
	...
	/* either raidz is fine, and the rr_zhe->handle can be deallocated */
	/* or raidz is down, and there's nothing that can be done on the offloader anyways */
	zoff_provider->raid.free(rr_zhe->handle);

	/* clean up columns because they are not removed by zoff_provider->raidz.free */
...
int zoff_cleanup_raidz(zio_t *zio, raidz_row_t *rr) {
	if (!zio || !rr) {
		return ZOFF_ERROR;
	}
	...
	/* bring data back to zio, which should place data into parent automatically */
	zoff_onload_abd_private(zio->io_abd, zio->io_abd->abd_size, B_FALSE, B_TRUE);
...
/* TODO: remove this - there is no reason to onload raidz parity columns except for debugging */
int zoff_onload_raidz(zio_t *zio, raidz_row_t *rr, boolean_t remove) {
	/* bring data back to zio, which should place data into parent automatically */
	zoff_onload_abd_private(zio->io_abd, zio->io_abd->abd_size, B_FALSE, remove);

	/* only bring parity columns back - data columns should have been brought back through zio->io_abd */
	for (int c = 0; c < rr->rr_firstdatacol; c++) {
		zoff_onload_abd_private(rr->rr_col[c].rc_abd, rr->rr_col[c].rc_size, B_FALSE, remove);
	}

	if (remove == B_TRUE) {
		return zoff_free_raidz_private(rr, B_FALSE);
	}

	return ZOFF_OK;
}

int zoff_free_raidz(raidz_row_t *rr) {
...
boolean_t zoff_all_zeros(void *key) {
	if (!zoff_provider) {
		return B_FALSE;
	}
...
int zoff_checksum_compute(abd_t *abd, enum zio_checksum alg, zio_byteorder_t order,
	...
	const int rc = zoff_provider->checksum.compute(alg, order, abd_zhe->handle, size, bp_cksum_zhe->handle,
...
int zoff_checksum_error(zoff_prop_t *zoff, enum zio_checksum alg, zio_byteorder_t order,
    abd_t *abd, uint64_t size, const blkptr_t *bp,
    int encrypted, int dedup, zio_bad_cksum_t *info) {
	if (!zoff_provider || !zoff_provider->checksum.error) {
		return ZOFF_FALLBACK;
	}
	...
	/* block pointer checksum should have been offloaded already */
	zhe_t *bp_cksum_zhe = zoff_hash_find_mapping(&ZOFF_HANDLES, &bp->blk_cksum);
	if (!bp_cksum_zhe) {
		zoff_hash_context_read_unlock(&ZOFF_HANDLES);
		return ZOFF_FALLBACK;
	}

	/* if mapping is not found, offload the abd */
	zhe_t *abd_zhe = zoff_hash_find_mapping(&ZOFF_HANDLES, abd);
	if (!abd_zhe) {
		zoff_hash_context_read_unlock(&ZOFF_HANDLES);
		return ZOFF_FALLBACK;
	...
	void *zbc_expected_zhe = NULL;
	void *zbc_actual_zhe = NULL;
	void *zbc_checksum_name_zhe = NULL;

	if (info != NULL) {
		zbc_expected_zhe = zoff_provider->alloc(ZFS_CHECKSUM_SIZE);
		zbc_actual_zhe = zoff_provider->alloc(ZFS_CHECKSUM_SIZE);
		zbc_checksum_name_zhe = zoff_provider->alloc(ZOFF_NAME_LEN);;
	}

	/* trigger checksum operation in provider/offloader */
	const int rc = zoff_provider->checksum.error(alg, order, abd_zhe->handle, bp_cksum_zhe->handle, encrypted, dedup, zbc_expected_zhe, zbc_actual_zhe, zbc_checksum_name_zhe);
...
int zoff_compress(enum zio_compress c, abd_t *src,
...
int zoff_raidz_gen(zio_t *zio, raidz_row_t *rr) {
	if (!rr) {
		return ZOFF_FALLBACK;
	}
...
int zoff_create_gang(abd_t *gang) {
	if (!zoff_provider) {
		return ZOFF_FALLBACK;
	}
...
int zoff_write_file(zfs_file_t *dst, abd_t *abd, ssize_t size,
	...
	const int rc = zoff_provider->write.file(dst, abd_zhe->handle, size, offset, resid, err);

	zoff_hash_context_read_unlock(&ZOFF_HANDLES);
	return rc;
}

int zoff_write_disk(struct block_device *bdev, zio_t *zio,
...
#ifdef _KERNEL
void zoff_disk_write_completion(void *zio_ptr, int error) {
